...
name: familles
description: >
  Fill the "Analyse spécifique / Familles" section of the Feu Vert Annecy monthly report
  from the comparatifCAv2_Famille CSV export. ALWAYS use when the user types /familles,
  or says "remplis les familles", "analyse les familles", "mets à jour les familles",
  or wants to populate the product-family breakdown table (CA n, CA n-1, Evol. CA,
  Marge n, Marge +/-, Qté n, Statut) in the rapport mensuel.
---
...
famille) et remplit la section **"Analyse spécifique / Familles"** du rapport mensuel,
incluant le tableau de données et les points clés automatiques.

---

## Workflow — Commande `/familles`
...
if not csv_files:
    raise FileNotFoundError("Aucun fichier comparatifCAv2_Famille*.csv trouvé dans monthly_recap/")
fichier_familles = csv_files[0]

# Rapport mensuel existant (créé par /chiffre-mensuel)
rapport_dir = str(find_dir("Rapport mensuel"))
rapports = glob.glob(os.path.join(rapport_dir, "rapport mensuel *.md"))
if not rapports:
    raise FileNotFoundError("Aucun rapport mensuel trouvé. Lancer /chiffre-mensuel d'abord.")
...
> Les noms suivent le format `rapport mensuel mars 2026.md`.
...
1. **Famille moteur** : la famille avec la plus forte croissance CA en valeur absolue (€)
...
5. **Synthèse globale** : tendance globale du mois (est-ce que la majorité des familles progressent ?)
...
Rédiger les points clés en **français**, de manière factuelle et orientée management.
...
Le fichier est placé dans `resources/Resources mensuelles/Familles/`.
