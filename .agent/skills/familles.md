---
description: familles hebdomadaire
---

ALWAYS use this skill when the user types "/familles" on a weekly report context. Execute the Python script
below in full. It parses the comparatifCAv2_Famille CSV from resources/familles/, fills the Familles table
with str.replace per row, then passes a compact summary to the AI for the "Points clés" section only.
Do NOT read the CSV yourself.
Other triggers: "remplis les familles hebdo", "analyse les familles semaine", "mets à jour les familles hebdo".

---

# Skill : Analyse par Familles — Rapport Hebdomadaire Feu Vert Annecy
...
familles_folder = str(find_dir("resources") / "familles")
csv_files       = glob.glob(os.path.join(familles_folder, "comparatifCAv2_Famille*.csv"))

assert csv_files, "ERREUR : aucun fichier comparatifCAv2_Famille*.csv trouvé dans resources/familles/"
...
> "Rédige 4 points clés de l'analyse par famille pour le rapport hebdomadaire Feu Vert Annecy.
